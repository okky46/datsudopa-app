...
1. 開始時: `start_raid_participation(session_id, raid_id, started_at)` RPC
2. 終了時: `finish_raid_participation(session_id, status, watched_seconds)` RPC
3. リザルト: `get_raid_companions(raid_id)` RPC

失敗分（ネットワーク/タイムアウト）は `raidSyncQueue`（AsyncStorage）に残り、ホーム表示時に順次再送。ただし公式窓外になったstartや不正なraid_idなど確定的に公式参加へ登録できないものは対応finishとともに除去し、ローカル実績は `unsynced` として残す。
...
- **raid_participations 書き込み**: 直接 upsert は不可。`start_raid_participation` が
  ...
  サーバー側で決定。公式時間内かどうかは **サーバー時刻** で判定し、`raid_id` はサーバー現在時刻のJST日付から作る `YYYY-MM-DD_22JST` と完全一致させるため、端末時計やsuffix変更を偽装できない。
